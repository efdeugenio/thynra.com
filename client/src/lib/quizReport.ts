...
export const STAGE_META: Record<
  Stage,
  { label: string; blurb: string; hex: string }
> = {
  awareness: {
    label: "Awareness",
...
export const STAGE_INSIGHT: Record<
  Stage,
...
export const STAGE_RECO: Record<Stage, string> = {
  awareness:
...
export const INDUSTRY_STATS: { value: string; label: string }[] = [
...
export const ACTION_PLAN: Record<
  Stage,
  { horizon: string; title: string; steps: string[] }[]
> = {
  awareness: [
    {
...
export function reportSummaryText(r: ReportData): string {
  const lvl = LEVELS[r.levelKey];
  return (
    `Front-Office AI Readiness assessment\n` +
    `Result: ${lvl.name} (score ${r.total}/24)\n` +
    `Awareness ${r.scores.awareness}/8, Conversion ${r.scores.conversion}/8, Retention ${r.scores.retention}/8\n` +
    `Weakest stage: ${STAGE_META[r.weakest].label}`
  );
}
